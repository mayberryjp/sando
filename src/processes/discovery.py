...
from src.database.localhosts import get_localhosts, update_localhosts
from src.integrations.dns import (
    dns_lookup,
)  # Import the dns_lookup function from dns.py
...
def do_discovery():
    ...
    elif config_dict.get("DiscoveryReverseDns", 0) == 1 and (not DNS_SERVERS):
        log_error(
            logger,
            "[ERROR] DiscoveryReverseDns is enabled but no DNS servers are configured in ApprovedLocalDnsServersList",
        )
        ...
        for result in nmap_results:
            ...
            if nmap_mac and nmap_mac != "Not available":
                ...
                if not current_mac or current_mac == "Not available":
                    log_info(
                        logger,
                        f"[INFO] Using Nmap-discovered MAC address for {ip}: {nmap_mac}",
                    )
...
if __name__ == "__main__":
    ...
    log_info(
        logger,
        f"[INFO] Pausing discovery process for {STARTUP_DELAY} seconds at startup",
    )
    ...
    log_info(logger, "[INFO] Discovery process started.")

    DISCOVERY_RUN_INTERVAL = config_dict.get(
