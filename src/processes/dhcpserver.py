...
class DHCPServer:
    def __init__(
        ...
        self.scopes = (
            scopes if scopes is not None else self._load_scopes_from_configuration()
        )
        ...
        self.registered_devices = {
            mac.lower(): ip for mac, ip in registered_devices.items()
        }
        ...
        log_info(
            self.logger,
            f"[INFO] Initialized DHCP server with {len(self.registered_devices)} registered devices",
        )
    ...
    def _load_scopes_from_configuration(self):
        ...
        try:
            ...
            log_error(
                self.logger,
                "[ERROR] configuration.get_config_settings() not available; no scopes loaded",
            )
            return []
        ...
        if not raw:
            log_warn(
                self.logger, "[WARN] LocalNetworks missing; no scopes will be used"
            )
            return []
    ...
    def _get_registered_ip(self, mac, packet):
        ...
        try:
            ...
            if not mac_found:
                ...
                log_info(
                    self.logger,
                    f"[INFO] Inserted new MAC address into localhosts database: {mac}",
                )
                insert_action(
                    f"New host detected: Assign a description and category for {mac}"
                )
                message = f"New Host Detected: {mac}"
                log_info(
                    self.logger,
                    f"[INFO] Sending NewHostsDetection alert for {DHCPServer.make_json_serializable(packet)}",
                )
            ...
            log_error(
                self.logger,
                f"[ERROR] Could not query or insert registered devices: {e}",
            )
        log_warn(
            self.logger, f"[WARN] Unregistered device attempted DHCP request: {mac}"
        )
        return None
    ...
    def _build_dhcp_packet(self, message_type, request_packet, offered_ip, scope):
        ...
        lease_time = scope.get(
            "lease_time", 86400
        )  # <-- Use scope lease_time or default
        ...
        if dns_servers:
            dns_bytes = b"".join(
                [self._ip_to_bytes(dns.strip()) for dns in dns_servers]
            )
            ...
            for ntp in ntp_servers:
                try:
                    ...
                    try:
                        ...
                        log_warn(
                            self.logger, f"[WARN] Could not resolve NTP server: {ntp}"
                        )
            if ntp_ips:
    ...
    def _handle_discover(self, packet, addr):
        ...
        if not assigned_ip:
            log_warn(
                self.logger, f"[WARN] Ignoring DISCOVER from unregistered MAC: {mac}"
            )
            return
        ...
        if not scope:
            log_warn(
                self.logger,
                f"[WARN] No matching scope for giaddr={packet['giaddr']} ip={assigned_ip}; dropping DISCOVER",
            )
            return
        ...
        if packet["giaddr"] and packet["giaddr"] != "0.0.0.0":
            ...
            mask = scope.get("subnet_mask")
            bcast = (
                self._calculate_broadcast(assigned_ip, mask)
                if mask
    ...
    def _handle_request(self, packet, addr):
        ...
        if DHCP_OPTION_REQUESTED_IP in packet["options"]:
            requested_ip = self._bytes_to_ip(
                packet["options"][DHCP_OPTION_REQUESTED_IP]
            )
        ...
        if not assigned_ip:
            log_warn(
                self.logger,
                f"[WARN] Unregistered device {mac} requested IP, sending NAK",
            )
        ...
        if requested_ip != assigned_ip:
            log_warn(
                self.logger,
                f"[WARN] Device {mac} requested {requested_ip} but registered for {assigned_ip}; sending NAK",
            )
            nak = self._build_dhcp_packet(DHCP_NAK, packet, "0.0.0.0", None)
            try:
                dest_addr = (
        ...
        if not scope:
            log_warn(
                self.logger,
                f"[WARN] No matching scope for giaddr={packet['giaddr']} ip={assigned_ip}; dropping REQUEST",
            )
            return
    ...
    def _handle_inform(self, packet, addr):
        ...
        if not assigned_ip:
            log_warn(
                self.logger, f"[WARN] Ignoring INFORM from unregistered MAC: {mac}"
            )
            return
        ...
        if not scope:
            log_warn(
                self.logger,
                f"[WARN] No matching scope for INFORM giaddr={packet['giaddr']} ip={client_ip}; dropping",
            )
            return
        ...
        try:
            if packet["giaddr"] and packet["giaddr"] != "0.0.0.0":
                ...
                mask = scope.get("subnet_mask")
                bcast = (
                    self._calculate_broadcast(client_ip, mask)
                    if mask
                    else "255.255.255.255"
                )
    ...
    def start(self):
        ...
        log_info(
            self.logger, "[INFO] Starting DHCP server (registered devices only)..."
        )
        try:
            ...
            log_info(
                self.logger,
                f"[INFO] DHCP server listening on {self.listen_ip}:{self.listen_port}",
            )
            log_info(self.logger, f"[INFO] Server IP: {self.server_ip}")
            log_info(
                self.logger,
                f"[INFO] Registered Devices: {len(self.registered_devices)}",
            )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not config_dict:
            log_error(
                logging(__name__), "[ERROR] Failed to load configuration settings"
            )

        if not config_dict.get("DhcpServer", 0):
            log_info(
                logging, "[INFO] DhcpServer is disabled in configuration. Exiting."
            )
        else:
            try:
                hosts = get_localhosts_all()
                ...
                log_info(
                    ...
                    f"[INFO] Loaded {len(registered_devices)} registered devices from localhosts database",
                )
            except Exception as e:
                log_error(
                    logging.getLogger(__name__),
                    f"[ERROR] Could not load registered devices from localhosts database: {e}",
                )
                registered_devices = {}
            ...
            try:
                ...
                log_info(
                    logging.getLogger(__name__),
                    "[INFO] KeyboardInterrupt received, shutting down DHCP server.",
                )
                server.stop()
