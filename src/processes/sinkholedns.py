...
from src.database.dnsqueries import insert_dns_queries_batch
...
class SinkholeResolver:
    def __init__(self, listen_ip="0.0.0.0", port=53, ttl=60, batch_interval_minutes=29):
        ...
        self.running = False
        self.stats = {"total_queries": 0, "unique_domains": set(), "clients": set()}
    ...
    def start(self):
        ...
        log_info(
            logger, f"[INFO] UDP DNS sinkhole listening on {self.listen_ip}:{self.port}"
        )
        ...
        log_info(
            logger, f"[INFO] TCP DNS sinkhole listening on {self.listen_ip}:{self.port}"
        )
        ...
        log_info(
            logger,
            f"[INFO] Started batch writer thread (interval: {self.batch_interval // 60} minutes)",
        )
    ...
    def stop(self):
        ...
        # Log statistics
        log_info(
            logger, f"[INFO] Statistics: {self.stats['total_queries']} total queries"
        )
    ...
    def batch_writer(self):
        ...
        while self.running:
            ...
            if current_time - self.last_batch_time >= self.batch_interval:
                ...
                log_info(
                    logger,
                    f"[INFO] Performed scheduled batch processing after {self.batch_interval // 60} minutes",
                )
    ...
    def process_queue(self, force=False):
        ...
        with self.query_lock:
            ...
            if (
                not force and queue_size < 100
            ):  # Only process if we have enough entries or forced
                return
        ...
        if current_batch:
            log_info(
                logger,
                f"[INFO] Writing batch of {len(current_batch)} DNS queries to database",
            )
            ...
            try:
                ...
                log_info(
                    logger,
                    f"[INFO] Batch processing complete. Success: {success_count}, Errors: {error_count}",
                )
    ...
    def handle_tcp(self):
        ...
        while self.running:
            try:
                ...
                threading.Thread(
                    target=self.process_tcp_request, args=(conn, client_ip), daemon=True
                ).start()
    ...
    def process_tcp_request(self, conn, client_ip):
        ...
        try:
            ...
            log_error(
                logger, f"[ERROR] Error processing TCP request from {client_ip}: {e}"
            )
        finally:
            conn.close()
    ...
    def process_dns_request(self, data, client_ip, udp_socket=None, client_addr=None):
        ...
        try:
            ...
            if len(self.query_queue) >= 10000:  # Process when queue gets very large
                ...
                threading.Thread(
                    target=self.process_queue, kwargs={"force": True}, daemon=True
                ).start()
            ...
            if self.stats["total_queries"] % 100 == 0:
                log_info(
                    logger,
                    f"[INFO] Processed {self.stats['total_queries']} queries (last: {client_ip}: {qname})",
                )
