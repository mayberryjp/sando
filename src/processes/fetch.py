    ...
    while True:
        try:
            ...
            if not config_dict:
                log_error(
                    logger,
                    "[ERROR] Failed to load configuration settings in Pihole fetch thread",
                )
    ...
    while True:
        ...
        try:
            if config_dict.get("SendDeviceClassificationsToHomelabApi", 0) > 0:
                log_info(
                    logger, "[INFO] Sending device classifications to Sando API..."
                )
                upload_all_client_definitions()
        ...
        try:
            if config_dict.get("SendConfigurationToCloudApi", 0) > 0:
                log_info(
                    logger, "[INFO] Sending instance configuration to Sando API..."
                )
                upload_configuration()
                log_info(logger, "[INFO] Instance configuration upload finished.")
                log_info(logger, "[INFO] Sending database metrics to Sando API...")
                upload_database_metrics()
        ...
        log_info(
            logger,
            f"[INFO] Sleeping for {fetch_interval} seconds before the next fetch.",
        )
...
if __name__ == "__main__":
    ...
    log_info(
        logger,
        f"[INFO] Starting fetcher, waiting {STARTUP_DELAY} seconds before starting processing",
    )
