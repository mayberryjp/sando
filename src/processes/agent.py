...
class NLPProcessor:
    ...
    def extract_with_basic_nlp(self, text):
        ...
        for endpoint in self.api_endpoints:
            ...
            keyword_score = sum(
                3 for keyword in endpoint["keywords"] if keyword.lower() in text
            )
            ...
            if method_verb in method_verbs:
                verb_score = sum(
                    2 for verb in method_verbs[method_verb] if verb in text
                )
                keyword_score += verb_score

            # Bonus for relevant entity presence
            if any(
                param in ["ip_address", "identifier"] for param in endpoint["params"]
            ):
                if re.search(r"\b(?:\d{1,3}\.){3}\d{1,3}\b", text):  # IP address
                    keyword_score += 3

            if (
                "domain" in endpoint["description"].lower()
                or "dns" in endpoint["description"].lower()
            ):
                if re.search(
            ...
            for i, keyword1 in enumerate(endpoint["keywords"]):
                if keyword1 in text:
                    ...
                    nearby_text = text[
                        max(0, keyword_pos - 30) : min(len(text), keyword_pos + 30)
                    ]
            ...
            if endpoint_scores[0][1] - endpoint_scores[1][1] < 2:
                ...
                for endpoint in top_endpoints:
                    for param in endpoint["params"]:
                        ...
                        if param in param_patterns and re.search(
                            param_patterns[param], text
                        ):
                            if endpoint == endpoint_scores[1][0]:
                                # Swap positions if second endpoint's param is found
                                endpoint_scores[0], endpoint_scores[1] = (
                                    endpoint_scores[1],
                                    endpoint_scores[0],
                                )
        ...
        log_info(
            logger,
            f"Selected intent: {best_endpoint['name']} with score: {endpoint_scores[0][1]}",
        )
        ...
        for param in best_endpoint["params"]:
            ...
            if param == "action_id":
                action_match = re.search(
                    r"\b(?:action|task)[\s:]?([a-zA-Z0-9_-]+)\b", text
                )
                if action_match:
            ...
            if param == "category_name":
                cat_match = re.search(
                    r"\b(?:category|type)[\s:]?([a-zA-Z0-9_-]+)\b", text
                )
                if cat_match:
            ...
            param_match = re.search(rf"{param}[:\s=]+([^\s,]+)", text)
            if param_match:
                params[param] = param_match.group(1)

        return {"endpoint": best_endpoint["name"], "params": params}
    ...
    def execute_api_call(self, endpoint_name, params):
        ...
        endpoint_def = next(
            (e for e in self.api_endpoints if e["name"] == endpoint_name), None
        )
        if not endpoint_def:
    ...
    def summarize_localhost_data(self, data):
        ...
        try:
            ...
            return {"original_data": data, "summary": summary.strip()}
        except Exception as e:
    ...
    def summarize_port_service_data(self, data):
        ...
        try:
            ...
            for protocol, service in data.items():
                ...
                if description:
                    protocols.append(
                        f"{service_name} ({protocol.upper()}) - {description}"
                    )
                else:
            ...
            return {"original_data": data, "summary": summary}
        except Exception as e:
    ...
    def process_request(self, text):
        ...
        if not endpoint:
            return {
                "success": False,
                "message": "Could not determine intent from your request",
            }
...
def start_interactive_cli():
    ...
    server_thread = threading.Thread(
        target=lambda: run(app, host="0.0.0.0", port=5000, quiet=True), daemon=True
    )
    server_thread.start()
        ...
        try:
            ...
            # Handle help command
                ...
                for endpoint in API_ENDPOINTS:
                    params = (
                        ", ".join(endpoint["params"]) if endpoint["params"] else "none"
                    )
                    print(
                        f"- {endpoint['name']}: {endpoint['description']} (Parameters: {params})"
                    )
                continue
...
if __name__ == "__main__":
    log_info(
        logger, "Starting NLP Agent with built-in processing only (OpenAI disabled)"
    )
