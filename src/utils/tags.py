...
from src.utils.locallogging import log_error, log_warn
...
def tag_ignorelist(record, ignorelist_entries):
    ...
    for (
        ...
        ignorelist_dst_port,
        ignorelist_protocol,
    ) in ignorelist_entries:

        src_match = (
            ignorelist_src_ip == record["src_ip"]
            or ignorelist_src_ip == record["dst_ip"]
            or ignorelist_src_ip == "*"
        )
        dst_match = (
            ignorelist_dst_ip == record["dst_ip"]
            or ignorelist_dst_ip == record["src_ip"]
            or ignorelist_dst_ip == "*"
        )
        port_match = (
            ignorelist_dst_port in (record["src_port"], record["dst_port"])
            or ignorelist_dst_port == "*"
        )
        protocol_match = (int(ignorelist_protocol) == record["protocol"]) or (
            ignorelist_protocol == "*"
        )
...
def tag_broadcast(record, broadcast_addresses):
    ...
    if (
        record["src_ip"] in broadcast_addresses
        or record["dst_ip"] in broadcast_addresses
    ):
        return "Broadcast;"
    else:
        return None
...
def tag_custom(record, tag_entries):
    ...
    for tag_name, tag_src_ip, tag_dst_ip, tag_dst_port, tag_protocol in tag_entries:
        try:
            ...
            src_match = (
                tag_src_ip == record["src_ip"]
                or tag_src_ip == record["dst_ip"]
                or tag_src_ip == "*"
            )
            dst_match = (
                tag_dst_ip == record["dst_ip"]
                or tag_dst_ip == record["src_ip"]
                or tag_dst_ip == "*"
            )

            # Port check - handle string conversion for wildcard
            port_match = tag_dst_port == "*" or tag_dst_port in (
                record["src_port"],
                record["dst_port"],
            )
            ...
            protocol_match = (
                tag_protocol == "*" or int(tag_protocol) == record["protocol"]
            )
