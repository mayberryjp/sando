...
def log_error(logger, message):
    ...
    if config_dict.get("SendErrorsToCloudApi", 0) == 1:
        if excluded_messages and any(
            excluded_msg in message for excluded_msg in excluded_messages
        ):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                log_warn(
                    logger,
                    f"[WARN] Failed to report error to cloud API: {response.status_code} {url}",
                )
        except Exception as e:
            log_warn(
                logger, f"[WARN] Failed to send error report to cloud API {url}: {e}"
            )
    else:
