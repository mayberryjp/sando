...
from src.database.dnsqueries import get_client_dns_queries
from src.database.localhosts import get_localhost_by_ip, get_localhosts
...
def export_client_definition(client_ip):
    ...
    try:
        ...
        log_error(
            logger, f"[ERROR] Failed to export client definition for {client_ip}: {e}"
        )
        return None
...
def upload_all_client_definitions():
    ...
    try:
        ...
        log_info(
            logger, f"[DEBUG] Number of IP addresses to process: {len(ip_addresses)}"
        )
        ...
        for ip_address in ip_addresses:
            ...
            try:
                ...
                if not client_data:
                    log_warn(
                        logger, f"[WARN] No client data generated for {ip_address}"
                    )
                    continue
        ...
        log_info(
            logger,
            f"[INFO] Upload complete. Success: {success_count}, Errors: {error_count}",
        )
...
def upload_configuration():
    ...
    try:
        ...
        api_url = (
            f"http://api.homelabids.com:8045/api/configurations/{instance_identifier}"
        )
        ...
        if response.status_code in (200, 201, 204):
            log_info(
                logger,
                f"[INFO] Successfully uploaded configuration for instance {instance_identifier}.",
            )
            return True
        else:
            log_error(
                logger,
                f"[ERROR] Failed to upload configuration: HTTP {response.status_code}",
            )
            return False

    except requests.RequestException as e:
        log_error(
            logger, f"[ERROR] Request failed while uploading configuration: {str(e)}"
        )
        return False
    except Exception as e:
        log_error(
            logger, f"[ERROR] Unexpected error while uploading configuration: {str(e)}"
        )
        return False
...
def upload_database_metrics():
    ...
    try:
        ...
        explore_db_size = (
            os.path.getsize(CONST_EXPLORE_DB) if os.path.exists(CONST_EXPLORE_DB) else 0
        )
        performance_db_size = (
            os.path.getsize(CONST_PERFORMANCE_DB)
            if os.path.exists(CONST_PERFORMANCE_DB)
            else 0
        )
        ...
        if response.status_code in (200, 201, 204):
            log_info(
                logger,
                f"[INFO] Successfully uploaded database metrics for instance {instance_identifier}.",
            )
            return True
        else:
            log_error(
                logger,
                f"[ERROR] Failed to upload database metrics: HTTP {response.status_code}",
            )
            return False

    except requests.RequestException as e:
        log_error(
            logger, f"[ERROR] Request failed while uploading database metrics: {str(e)}"
        )
        return False
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while uploading database metrics: {str(e)}",
        )
        return False
