...
def send_telegram_message(message, flow):
    ...
    if (
        config_dict["TelegramBotToken"]
        and config_dict["TelegramChatId"]
        and config_dict["TelegramEnabled"]
    ):
        try:

            # Create header with warning emoji and site name
            header = f"⚠️ SANDO Security Alert - {SITE}\n\n"
            ...
            if response.status_code == 200:
                log_info(logger, "[INFO] Telegram message sent successfully.")
            else:
                log_error(
                    logger,
                    f"[ERROR] Failed to send Telegram message. Status code: {response.status_code}, Response: {response.text}",
                )
        except Exception as e:
            log_error(
                logger,
                f"[ERROR] Exception occurred while sending Telegram message: {e}",
            )
...
def send_test_telegram_message():
    ...
    if (
        ...
        try:
            ...
            response = requests.post(url, json=payload)
            if response.status_code == 200:
                log_info(logger, "[INFO] Test Telegram message sent successfully.")
            else:
                log_error(
                    logger,
                    f"[ERROR] Failed to send test Telegram message. Status code: {response.status_code}, Response: {response.text}",
                )
        except Exception as e:
            log_error(
                logger,
                f"[ERROR] Exception occurred while sending test Telegram message: {e}",
            )
    else:
        log_warn(
            logger,
            "[WARN] TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID is not set. Skipping test Telegram message.",
        )
