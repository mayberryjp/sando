...
from src.const import CONST_SITE, IS_CONTAINER
from src.database.configuration import get_config_settings, get_local_network_cidrs
from src.database.geolocation import (
    get_all_geolocations,
    get_country_by_ip_int,
    insert_geolocation,
)
from src.utils.locallogging import log_error, log_info, log_warn
from src.utils.network import ip_network_to_range, ip_to_int
...
def create_geolocation_db():
    ...
    # Return and do nothing if api_key is an empty string
    if api_key == "":
        log_info(
            logger,
            "[INFO] No MaxMind API key provided, skipping geolocation DB creation.",
        )
        return
    ...
    try:
        ...
        log_info(
            logger, "[INFO] Attempting to download GeoLite2 database from MaxMind..."
        )

        try:
            # Download blocks CSV
            blocks_response = requests.get(
                blocks_csv_url.format(api_key=api_key), stream=True, timeout=30
            )
            locations_response = requests.get(
                locations_csv_url.format(api_key=api_key), stream=True, timeout=30
            )

            if (
                blocks_response.status_code != 200
                or locations_response.status_code != 200
            ):
                download_successful = False
                log_warn(
                    logger,
                    "[WARN] Failed to download geolocation data. Will attempt to use local files.",
                )
            else:
            ...
            log_warn(
                logger,
                f"[WARN] Error during download: {e}. Will attempt to use local files.",
            )
        ...
        if not os.path.exists(blocks_csv_path) or not os.path.exists(
            locations_csv_path
        ):
            log_error(
                logger,
                "[ERROR] Required CSV files not found. Cannot create geolocation database.",
            )
            return

        log_info(
            logger,
            f"[INFO] Using geolocation data files: {blocks_csv_path} and {locations_csv_path}",
        )
        ...
        log_info(
            logger, "[INFO] Populating the SQLite database with country blocks data..."
        )
        geolocation_batch = []
        total_records = 0
        ...
        with open(blocks_csv_path, "r", encoding="utf-8") as blocks_file:
            ...
            for row in reader:
                ...
                country_name = locations.get(
                    geoname_id, None
                )  # Get the country name from the locations dictionary

                # Add to batch
                geolocation_batch.append(
                ...
                if len(geolocation_batch) >= BATCH_SIZE:
                    ...
                    log_info(
                        logger,
                        f"[INFO] Inserted {total_records} geolocation records so far...",
                    )
        ...
        if geolocation_batch:
            ...
            log_info(
                logger,
                f"[INFO] Inserted {total_records} total MaxMind geolocation records",
            )
            geolocation_batch = []  # Clear the batch

        # After processing CSV files, add LOCAL_NETWORKS
        log_info(logger, "[INFO] Adding LOCAL_NETWORKS to geolocation database...")
                ...
                if "=" in pair:
                    ...
                    if start_ip is not None:
                        local_networks_batch.append(
                            (network, start_ip, end_ip, netmask, site_name)
                        )
        ...
        local_networks_batch.append(
            ("224.0.0.0/4", "3758096384", "4026531839", "4", "Multicast")
        )
        log_info(logger, "[INFO] Adding Other Networks to geolocation database...")
        ...
        if local_networks_batch:
            ...
            log_info(
                logger,
                f"[INFO] Added {success_count} local and other network records to geolocation database",
            )

        log_info(logger, "[INFO] Geolocation database created successfully.")
