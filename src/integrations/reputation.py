...
from src.database.reputation import get_all_reputation_records, insert_reputation
...
def import_reputation_list(config_dict):
    ...
    reputation_url = config_dict.get(
    ...
    try:
        ...
        log_info(
            logger, f"[INFO] Downloading IP bad reputation list from {reputation_url}"
        )
        ...
        for line in netset_data:
            ...
            try:
                ...
                log_error(
                    logger, f"[ERROR] Invalid network entry in reputation list: {line}"
                )

        log_info(
            logger,
            f"[INFO] Imported {len(processed_networks)} networks into the reputation table.",
        )
