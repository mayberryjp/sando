...
from src.database.dnsqueries import (
...
def resolve_empty_dns_responses(config_dict):
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Found {len(empty_queries)} DNS queries with empty responses",
        )
        ...
        for query in empty_queries:
            ...
            # Skip if any label is longer than 63 octets
            if any(len(label) > 63 for label in domain.split(".")):
                log_warn(
                    logger,
                    f"[WARN] Skipping DNS query with label > 63 octets: {domain}",
                )
                continue
            ...
            try:
                ...
                if success:
                    ...
                    log_warn(
                        logger,
                        f"[WARN] Failed to update response for {domain} ({query_type})",
                    )
                    stats["failed"] += 1
        ...
        log_info(
            logger,
            f"[INFO] DNS resolution complete. Processed: {stats['processed']}, "
...
def dns_lookup(ip_addresses, dns_servers, config_dict):
    ...
    resolver_timeout = (
        config_dict["DnsResolverTimeout"] if "DnsResolverTimeout" in config_dict else 3
    )
    resolver_retries = (
        config_dict["DnsResolverRetries"] if "DnsResolverRetries" in config_dict else 1
    )
    ...
    for ip in ip_addresses:
        ...
        try:
            ...
            results.append({"ip": ip, "dns_hostname": hostname})

        except dns.resolver.NXDOMAIN:
            results.append({"ip": ip, "dns_hostname": "NXDOMAIN"})
            insert_action(
                f"You have an IP address without a reverse DNS entry. It is recommeneded to inventory all your local network devices in your local DNS. In Pi-hole, you can do this by going to Settings > Local DNS Records. Affected IP address is {ip}. DNS response was 'NXDOMAIN'."
            )
        except dns.resolver.Timeout:
            results.append({"ip": ip, "dns_hostname": "TIMEOUT"})
        except dns.resolver.NoNameservers:
            results.append({"ip": ip, "dns_hostname": "NONAMESERVER"})
        except Exception:
            results.append({"ip": ip, "dns_hostname": "ERROR"})

    log_info(logger, "[INFO] DNS discovery finished")
    return results
