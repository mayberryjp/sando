...
from src.database.configuration import get_local_network_cidrs, get_routers
...
def local_flows_detection(rows, config_dict):
    ...
    log_info(
        logger,
        "[INFO] Detecting flows for the same local networks going through the router",
    )
    for row in rows:
        (
            src_ip,
        ...
        if is_src_local and is_dst_local:
            log_info(
                logger, f"[INFO] Flow involves two local hosts: {src_ip} and {dst_ip}"
            )
    ...
    log_info(
        logger,
        "[INFO] Finished detecting flows for the same local network going through the router",
    )
