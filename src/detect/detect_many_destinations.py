...
def detect_many_destinations(rows, config_dict):
    ...
    for row in rows:
        ...
        if src_ip not in source_stats:
            source_stats[src_ip] = {"destinations": set(), "ports": set(), "flow": row}
    ...
    for src_ip, stats in source_stats.items():
        ...
        if unique_dests > dest_threshold:
            ...
            message = (
                f"Host Connecting to Many Destinations:\n"
                f"Source IP: {src_ip}\n"
            ...
            log_info(
                logger,
                f"[INFO] Excessive destinations detected from {src_ip}: {unique_dests} destinations",
            )
