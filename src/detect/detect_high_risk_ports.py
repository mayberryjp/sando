...
def detect_high_risk_ports(rows, config_dict):
    ...
    approved_destinations = set(
        config_dict.get("ApprovedHighRiskDestinations", "").split(",")
    )
    ...
    for row in rows:
        ...
        if dst_port in high_risk_ports:
            ...
            message = (
                f"High-Risk Port Traffic Detected:\n"
                f"Source: {src_ip}\n"
            ...
            log_info(
                logger,
                f"[INFO] High-risk port traffic detected: {src_ip} -> {dst_ip}:{dst_port} ({service_name})",
            )
    ...
    log_info(
        logger,
        f"[INFO] Completed high-risk port detection. Found {matches} matches in {total} flows",
    )
