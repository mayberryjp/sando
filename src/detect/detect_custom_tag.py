...
def detect_custom_tag(rows, config_dict):
    ...
    alert_tags = set(
        tag.strip()
        for tag in config_dict.get("AlertOnCustomTagList", "").split(",")
        if tag.strip()
    )
    ...
    for row in rows:
        ...
        if matching_tags:
            ...
            log_info(
                logger,
                f"[INFO] Custom tag alert detected: {src_ip} -> {dst_ip}:{dst_port} Tags: {', '.join(matching_tags)} ",
            )
