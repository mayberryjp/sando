...
def detect_new_outbound_connections(rows, config_dict):
    ...
    log_info(logger, "[INFO] Preparing to detect new outbound connections")
    ...
    try:
        ...
        for row in rows:
            ...
            if is_src_local and dst_port < src_port:
                ...
                alert_id = (
                    f"{src_ip}_{dst_ip}_{protocol}_{dst_port}_NewOutboundDetection"
                )

                message = (
                    f"New outbound connection detected:\n"
                ...
                log_info(
                    logger,
                    f"[INFO] New outbound connection detected: {src_ip} -> {dst_ip}:{dst_port}",
                )
    ...
    log_info(logger, "[INFO] Finished detecting new outbound connections")
