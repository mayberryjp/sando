...
def detect_vpn_traffic(rows, config_dict):
    ...
    log_info(logger, "[INFO] Started detecting VPN protocol usage")
    ...
    for row in rows:
        ...
        message = (
            f"Potential VPN Traffic Detected:\n"
            f"Source: {src_ip}\n"
        ...
        log_info(
            logger,
            f"[INFO] Potential VPN traffic detected: {src_ip} -> {dst_ip}:{dst_port} ({proto_name})",
        )
    ...
    log_info(logger, "[INFO] Finished detecting VPN protocol usage")
