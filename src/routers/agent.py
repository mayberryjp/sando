...
from src.database.ipasn import get_asn_for_ip
from src.integrations.dns import dns_lookup

# Import geolocation function
from src.integrations.geolocation import lookup_ip_country
...
def setup_agent_routes(app):
    ...
    def investigate_ip(ip_address):
        ...
        try:
            ...
            if DNS_SERVERS and config_dict.get("DiscoveryReverseDns", 0) > 0:
                ...
                log_info(
                    logger,
                    "[INFO] DNS lookup skipped - no DNS servers configured or discovery disabled",
                )
            ...
            if geo_result:
                ...
                log_info(
                    logger, f"[INFO] No geolocation result found for IP: {ip_address}"
                )

            response.content_type = "application/json"
            log_info(
                logger, f"[INFO] Successfully investigated IP address: {ip_address}"
            )
            return json.dumps(result, indent=2)

        except Exception as e:
            log_error(
                logger, f"[ERROR] Failed to investigate IP address {ip_address}: {e}"
            )
            response.status = 500
            return {"error": str(e)}
    ...
    def classify_client_api(ip_address):
        ...
        try:
            ...
            from src.database.common import get_machine_unique_identifier_from_db
            ...
            if not machine_identifier:
                log_error(
                    logger,
                    "[ERROR] Failed to get machine identifier for classification",
                )
                response.status = 500
