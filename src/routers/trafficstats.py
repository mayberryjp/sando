...
def setup_trafficstats_routes(app):
    ...
    def get_all_ips_traffic_status_route():
        ...
        try:
            ...
            log_info(
                logger,
                f"[INFO] Successfully retrieved traffic status for {len(ip_traffic_status)} IPs ({active_count} active)",
            )
    ...
    def get_traffic_stats(ip_address):
        ...
        try:
            ...
            if traffic_stats:
                ...
                response.content_type = "application/json"
                log_warn(
                    logger, f"[WARN] No traffic stats found for IP address {ip_address}"
                )
