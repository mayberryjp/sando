    ...
    def ignorelist():
        ...
        if request.method == "GET":
            try:
                ...
                log_info(
                    logger,
                    f"[INFO] Fetched {len(formatted_entries)} ignorelist entries successfully",
                )
            ...
            if (
                ...
                response.status = 400
                return {
                    "error": "Required fields missing (ignorelist_id, src_ip, dst_ip, dst_port, dst_protocol)"
                }

            try:

                insert_ignorelist_entry(
                    ignorelist_id, src_ip, dst_ip, dst_port, protocol, src_port=src_port
                )

                response.content_type = "application/json"
                log_info(
                    logger,
                    f"[INFO] Added new ignorelist entry: {ignorelist_id} {src_ip} -> {dst_ip}:{dst_port}/{protocol}",
                )
    ...
    def get_ignorelist_by_ip(ip_address):
        ...
        try:
            ...
            if ignorelist_entries is None:
                log_error(
                    logger,
                    f"[ERROR] Failed to retrieve ignorelist entries for IP {ip_address}",
                )
                response.status = 500
                return {"error": "Failed to retrieve ignorelist entries"}
            ...
            log_error(
                logger,
                f"[ERROR] Failed to fetch ignorelist entries for IP {ip_address}: {e}",
            )
            response.status = 500
            return {"error": str(e)}
