...
from src.database.alerts import (
...
def setup_alerts_routes(app):
    ...
    def delete_alerts_for_ip(ip_address):
        ...
        try:
            ...
            from src.database.alerts import delete_alerts_by_ip
            ...
            if success:
                ...
                log_error(
                    logger,
                    f"[ERROR] Failed to delete alerts for IP address: {ip_address}",
                )
                response.status = 500
                return {
                    "success": False,
            ...
            log_error(
                logger,
                f"[ERROR] Error deleting alerts for IP address {ip_address}: {e}",
            )
            response.status = 500
    ...
    def get_alerts_by_category_api(category_name):
        ...
        try:
            ...
            from src.database.alerts import get_alerts_by_category
            ...
            if not alerts:
                log_info(
                    logger, f"[INFO] No alerts found for category: {category_name}"
                )
                return json.dumps([])
            ...
            log_info(
                logger,
                f"[INFO] Retrieved {len(alerts)} alerts for category {category_name}",
            )
            ...
            log_error(
                logger,
                f"[ERROR] Database error fetching alerts for category {category_name}: {e}",
            )
            response.status = 500
            return {"error": str(e)}
        except Exception as e:
            log_error(
                logger,
                f"[ERROR] Failed to get alerts for category {category_name}: {e}",
            )
            response.status = 500
            return {"error": str(e)}
    ...
    def alerts():
        ...
        if request.method == "GET":
            try:
                ...
                log_info(
                    logger,
                    f"[INFO] Fetched {len(formatted_alerts)} alerts successfully",
                )
    ...
    def get_recent_alerts_by_ip_api(ip_address):
        ...
        try:
            ...
            if not alerts:
                log_info(
                    logger,
                    f"[INFO] No recent alerts found for IP address: {ip_address}",
                )
                response.content_type = "application/json"
                return json.dumps([])
            ...
            for alert in alerts:
                alert["local_description"] = localhost_descriptions.get(
            ...
            log_info(
                logger,
                f"[INFO] Retrieved {len(alerts)} recent alerts for IP address {ip_address}",
            )
            return json.dumps(alerts, indent=2)

        except Exception as e:
            log_error(
                logger, f"[ERROR] Failed to get recent alerts for IP {ip_address}: {e}"
            )
            response.status = 500
            return {"error": str(e)}
    ...
    def summarize_alerts_by_ip_address(ip_address):
        ...
        try:
            ...
            rows = get_hourly_alerts_summary(
            ...
            log_error(
                logger,
                f"[ERROR] Database error summarizing alerts for IP {ip_address}: {e}",
            )
            response.status = 500
            return {"error": str(e)}
        except Exception as e:
            log_error(
                logger, f"[ERROR] Failed to summarize alerts for IP {ip_address}: {e}"
            )
            response.status = 500
            return {"error": str(e)}
    ...
    def get_alerts_by_ip(ip_address):
        ...
        try:
            ...
            log_info(
                logger,
                f"[INFO] Retrieved {len(alerts)} alerts for IP address {ip_address}",
            )
            return json.dumps(alerts, indent=2)
            ...
            log_error(
                logger,
                f"[ERROR] Database error fetching alerts for IP {ip_address}: {e}",
            )
            response.status = 500
            return {"error": str(e)}
        except Exception as e:
    ...
    def delete_all_alerts():
        ...
        try:
            ...
            # Delete all alerts
            delete_all_records("alerts")

            response.content_type = "application/json"

            return {"message": "All alerts deleted successfully records)"}
