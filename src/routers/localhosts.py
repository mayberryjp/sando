...
from src.database.localhosts import (
    ...
    get_localhost_by_ip,
    get_localhosts_all,
    update_localhost_firewall_interface,
...
def setup_localhosts_routes(app):
    ...
    def localhosts():
        ...
        try:
            ...
            log_info(
                logger,
                f"[INFO] Fetched {len(localhosts_data)} local hosts successfully",
            )
    ...
    def modify_localhost(ip_address):
        ...
        if request.method == "PUT":
            ...
            firewall_interface_name = data.get("firewall_interface_name")
            ...
            try:
                ...
                if firewall_interface_name is not None:
                    update_localhost_firewall_interface(ip_address, firewall_interface_name)
    ...
    def delete_localhost(ip_address):
        ...
        try:
            ...
            return {
                "message": f"Local host with IP address {ip_address} deleted successfully"
            }
        except Exception as e:
            log_error(
                logger,
                f"[ERROR] Error deleting local host with IP address {ip_address}: {e}",
            )
            response.status = 500
            return {"error": str(e)}
        except Exception as e:
            log_error(
                logger,
                f"[ERROR] Unexpected error deleting local host with IP address {ip_address}: {e}",
            )
            response.status = 500
            return {"error": str(e)}
        ...
        try:
            ...
            if host_record:
                ...
                localhost_dict = {
                    ...
                    "last_seen": host_record[18],
                    "last_dhcp_discover": host_record[19],
                    "whitelisted": host_record[20],
                    "total_packets_src": host_record[21],
                    "total_packets_dst": host_record[22],
                    "total_bytes_src": host_record[23],
                    "total_bytes_dst": host_record[24],
                    "ip6_address": host_record[25],
                    "firewall_interface_name": host_record[26] if len(host_record) > 26 else None,
                }

                response.content_type = "application/json"
                log_info(
                    logger,
                    f"[INFO] Fetched local host details for IP address: {ip_address}",
                )
                return json.dumps(localhost_dict, indent=2)
            else:
                log_warn(
                    logger, f"[WARN] No local host found for IP address: {ip_address}"
                )
                response.status = 404
            ...
            log_error(
                logger,
                f"[ERROR] Failed to fetch local host for IP address {ip_address}: {e}",
            )
            response.status = 500
            return {"error": str(e)}
    ...
    def toggle_localhost_alerts(ip_address):
        ...
        try:
            ...
            if not data or "alerts_enabled" not in data:
                ...
                return {
                    "success": False,
                    "error": "Missing required field: alerts_enabled",
                }
            ...
            from src.database.localhosts import update_localhost_alerts_enabled
            ...
            if success:
                ...
                log_info(
                    logger,
                    f"[INFO] Updated alerts_enabled to {alerts_enabled} for IP address: {ip_address}",
                )
                return {
                    "success": True,
                    "ip_address": ip_address,
                ...
                log_warn(
                    logger,
                    f"[WARN] Failed to update alerts_enabled for IP address: {ip_address}",
                )
                response.status = 404
                return {
                    "success": False,
            ...
            log_error(
                logger,
                f"[ERROR] Failed to update alerts_enabled for IP address {ip_address}: {e}",
            )
            response.status = 500
            return {"success": False, "error": str(e)}
