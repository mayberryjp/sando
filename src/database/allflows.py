...
def update_all_flows(rows, config_dict):
    ...
    if conn:
        try:
            ...
            conn.commit()
            log_info(logger, f"[INFO] Updated allflows with {len(rows)} rows.")
        except sqlite3.Error as e:
            log_error(logger, f"[ERROR] Error updating allflows: {e}")
        finally:
...
def update_tag_to_allflows(table_name, tag, src_ip, dst_ip, dst_port):
    ...
    if not conn:
        log_error(logger, "[ERROR] Unable to connect to table: allflows")
    ...
    try:
        ...
        )  # Get the existing tag or default to an empty string
        ...
        log_info(
            logger,
            f"[INFO] Tag '{tag}' added to flow: {src_ip} -> {dst_ip}:{dst_port}. Updated tag: '{updated_tag}'",
        )
        return True
    except sqlite3.Error as e:
...
def get_flows_by_source_ip(src_ip):
    ...
    try:
        ...
        rows, _ = run_timed_query(
            ...
            query,
            params=(src_ip,),
            description="get_flows_by_source_ip",
            fetch_all=True,
        )
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving flows for source IP {src_ip}: {e}",
        )
        return []
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving flows for source IP {src_ip}: {e}",
        )
        return []
    finally:
    ...
    try:
        ...
        query = """
               WITH ConnectionPairs AS (
                ...
                AND row_tags not like '%Broadcast;%'
                AND row_tags not like '%Multicast;%'
                AND row_tags not like '%LinkLocal;%'
        ...
        log_info(
            logger,
            f"[INFO] Identified {len(restructured_rows)} potential dead connections.",
        )
        return restructured_rows

    except sqlite3.Error as e:
        log_error(
            logger, f"[ERROR] Database error while querying dead connections: {e}"
        )
        return []
    except Exception as e:
        log_error(
            logger, f"[ERROR] Unexpected error while querying dead connections: {e}"
        )
        return []
    finally:
        if "conn" in locals() and conn:
...
def get_tag_statistics(local_ip):
    ...
    try:
        ...
        if not conn:
            log_error(
                logger, "[ERROR] Unable to connect to database for tag statistics."
            )
            return {}
        ...
        rows, execution_time = run_timed_query(
            ...
            params=(
                local_ip,
                local_ip,
            ),  # Pass local_ip twice for src_ip and dst_ip conditions
        ...
        log_info(
            logger,
            f"[INFO] Retrieved statistics for {len(tag_stats)} unique tags for IP {local_ip} in {execution_time:.2f} ms",
        )
        return tag_stats
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving tag statistics for IP {local_ip}: {e}",
        )
        return {}
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving tag statistics for IP {local_ip}: {e}",
        )
        return {}
    finally:
        if "conn" in locals() and conn:
...
def apply_ignorelist_entry(ignorelist_id, src_ip, dst_ip, dst_port, protocol):
    ...
    try:
        ...
        if dst_port != "*":
            ...
            flow_params.extend(
                [dst_port, dst_port]
            )  # Add the port twice for both conditions
        ...
        flow_where_clause = (
            " AND ".join(flow_where_conditions) if flow_where_conditions else "1=1"
        )
        ...
        log_info(
            logger,
            f"[INFO] Applied ignorelist entry {ignorelist_id}: Updated {flows_updated} flows",
        )
