...
def get_hourly_alerts_summary(ip_address, start_time=None):
    ...
    try:
        ...
        hourly_summary, query_time = run_timed_query(
            ...
            description="get_hourly_alerts_summary",
        )

        log_info(
            logger,
            f"[INFO] Retrieved {len(hourly_summary)} hourly alert entries for IP {ip_address} in {query_time:.2f} ms",
        )
        return hourly_summary

    except sqlite3.Error as e:
        log_error(
            logger, f"[ERROR] Database error while retrieving hourly alert summary: {e}"
        )
        return []
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving hourly alert summary: {e}",
        )
        return []
    finally:
...
def get_all_alerts_by_ip(ip_address):
    ...
    try:
        ...
        rows, query_time = run_timed_query(
            cursor, alerts_query, (ip_address,), description="get_all_alerts_for_ip"
        )
        ...
        log_info(
            logger,
            f"[INFO] Retrieved {len(alerts)} alerts for IP address {ip_address} in {query_time:.2f} ms",
        )
        return alerts
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving alerts for IP {ip_address}: {e}",
        )
        return []
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving alerts for IP {ip_address}: {e}",
        )
        return []
    finally:
        if "conn" in locals() and conn:
...
def get_alert_count_by_id(alert_id):
    ...
    try:
        ...
        log_error(
            logger,
            f"[ERROR] Database error while counting alerts with ID {alert_id}: {e}",
        )
        return 0
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while counting alerts with ID {alert_id}: {e}",
        )
        return 0
    finally:
        if "conn" in locals() and conn:
...
def get_recent_alerts_database():
    ...
    try:
        ...
        rows, query_time = run_timed_query(
            cursor, recent_alerts_query, description="get_recent_alerts_database"
        )

        log_info(
            logger, f"[INFO] Retrieved {len(rows)} recent alerts in {query_time:.2f} ms"
        )
        return rows
        ...
        log_error(
            logger, f"[ERROR] Unexpected error while retrieving recent alerts: {e}"
        )
        return []
    finally:
        if "conn" in locals() and conn:
...
def delete_alert_database(alert_id):
    ...
    try:
        ...
        if cursor.rowcount > 0:
            conn.commit()
            log_info(
                logger, f"[INFO] Alert with ID {alert_id} was successfully deleted."
            )
            return True
        else:
        ...
        log_error(
            logger, f"[ERROR] Database error while deleting alert {alert_id}: {e}"
        )
        return False
    except Exception as e:
        log_error(
            logger, f"[ERROR] Unexpected error while deleting alert {alert_id}: {e}"
        )
        return False
    finally:
        if "conn" in locals() and conn:
...
def log_alert_to_db(
    ...
    try:
        ...
        cursor.execute("SELECT times_seen FROM alerts WHERE id = ?", (alert_id_hash,))
        row = cursor.fetchone()
        if row and row[0] == 1:
            operation = "insert"
            log_info(
                logger,
                f"[INFO] Alert logged to database for IP: {ip_address}, Category: {category} ({operation}).",
            )
        else:
            operation = "update"
...
def get_alerts_summary():
    ...
    try:
        ...
        total_count_result, count_query_time = run_timed_query(
            cursor, total_count_query, description="get_alerts_summary_count"
        )
        ...
        log_info(
            logger,
            f"[INFO] Total alerts: {total_count} (query took {count_query_time:.2f} ms)",
        )
        log_info(
            logger,
            f"[INFO] Breakdown by category (query took {category_query_time:.2f} ms):",
        )
...
def get_recent_alerts_by_ip(ip_address):
    ...
    try:
        ...
        rows, query_time = run_timed_query(
            cursor, alerts_query, (ip_address,), description="get_recent_alerts_by_ip"
        )
        ...
        log_info(
            logger,
            f"[INFO] Retrieved {len(alerts)} recent alerts for IP address {ip_address} in {query_time:.2f} ms",
        )
        return alerts
...
def get_all_alerts():
    ...
    try:
        ...
        rows, query_time = run_timed_query(
            cursor, "SELECT * FROM alerts", description="get_all_alerts"
        )
        ...
        log_info(
            logger,
            f"[INFO] Retrieved {len(alerts)} alerts from the database in {query_time:.2f} ms",
        )
        return alerts
...
def get_alerts_by_category(category_name):
    ...
    try:
        ...
        rows, query_time = run_timed_query(
            ...
            category_query,
            (category_name,),
            description="get_alerts_by_category",
        )

        log_info(
            logger,
            f"[INFO] Retrieved {len(rows)} alerts for category '{category_name}' in {query_time:.2f} ms",
        )
        return rows
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving alerts for category '{category_name}': {e}",
        )
        return []
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving alerts for category '{category_name}': {e}",
        )
        return []
    finally:
        if "conn" in locals() and conn:
...
def update_alert_acknowledgment(alert_id, acknowledged):
    ...
    try:
        ...
        cursor.execute(
            "UPDATE alerts SET acknowledged = ? WHERE id = ?", (acknowledged, alert_id)
        )
        ...
        if cursor.rowcount > 0:
            ...
            log_info(
                logger,
                f"[INFO] Alert {alert_id} acknowledged status updated to {acknowledged}.",
            )
            return True
        else:
        ...
        log_error(
            logger, f"[ERROR] Database error while updating alert acknowledgment: {e}"
        )
        return False
    except Exception as e:
        log_error(
            logger, f"[ERROR] Unexpected error while updating alert acknowledgment: {e}"
        )
        return False
    finally:
        if "conn" in locals() and conn:
...
def get_all_alerts_by_category(category):
    ...
    try:
        ...
        rows, query_time = run_timed_query(
            ...
            """,
            (category,),
            description="get_all_alerts_by_category",
        )
        ...
        log_info(
            logger,
            f"[INFO] Retrieved {len(alerts)} alerts for category {category} in {query_time:.2f} ms",
        )
        return alerts
    finally:
        if "conn" in locals() and conn:
    ...
    try:
        ...
        if dst_ip != "*":
            where_conditions.append(
                "(json_extract(flow, '$[0]') = ? OR json_extract(flow, '$[1]') = ?)"
            )
        ...
        if dst_port != "*":
            where_conditions.append(
                "(CAST(json_extract(flow, '$[2]') AS TEXT) = ? OR CAST(json_extract(flow, '$[3]') AS TEXT) = ?)"
            )
        ...
        log_info(
            logger,
            f"[INFO] Executing alert deletion for ignorelist entry {query} and params {params}",
        )
        log_info(
            logger,
            f"[INFO] Filter criteria: src_ip={src_ip}, dst_ip={dst_ip}, dst_port={dst_port}, protocol={protocol}",
        )
        ...
        log_info(
            logger,
            f"[INFO] Applied ignorelist entry {ignorelist_id}: Deleted {alerts_deleted} alerts",
        )
        return alerts_deleted
        ...
        log_error(
            logger,
            f"[ERROR] Database error while applying ignorelist entry {ignorelist_id}: {e}",
        )
        return 0
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while applying ignorelist entry {ignorelist_id}: {e}",
        )
        return 0
    finally:
