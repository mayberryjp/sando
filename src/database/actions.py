...
def get_all_actions():
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Retrieved {len(actions)} actions from the database in {query_time:.2f} ms",
        )
        return actions
...
def update_action_acknowledged(action_id):
    ...
    try:
        ...
        start_time = time.time()
        cursor.execute(
            "UPDATE actions SET acknowledged = 1 WHERE action_id = ?", (action_id,)
        )
        conn.commit()
        query_time = (time.time() - start_time) * 1000
        log_info(
            logger,
            f"[INFO] Updated acknowledged field for action ID: {action_id} in {query_time:.2f} ms",
        )
...
def update_action_acknowledged_all():
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Updated acknowledged field for all actions in {query_time:.2f} ms",
        )
        return cursor.rowcount > 0  # Return True if any rows were affected
