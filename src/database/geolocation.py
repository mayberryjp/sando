...
def insert_geolocation(rows):
    ...
    try:
        ...
        conn.commit()
        logger.info(
            f"[INFO] Successfully inserted {success_count} of {len(rows)} geolocation records"
        )
...
def get_all_geolocations():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT network, start_ip, end_ip, netmask, country_name FROM geolocation"
        )
...
def get_country_by_ip_int(ip_int):
    ...
    try:
        ...
        rows, _ = run_timed_query(
            ...
            query,
            params=(ip_int,),
            description="get_country_by_ip_int",
            fetch_all=True,
        )
        if rows:
            return rows[0][0]
