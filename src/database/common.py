...
from src.database.configuration import update_config_setting
...
from src.database.localhosts import get_average_threat_score
from src.utils.locallogging import (
...
def check_update_database_schema(config_dict):
    ...
    try:
        ...
        if current_schema != str(CONST_DATABASE_SCHEMA_VERSION):
            log_info(
                logger,
                f"[INFO] Database schema needs update: {current_schema} → {CONST_DATABASE_SCHEMA_VERSION}",
            )
            ...
            if update_database_schema(current_schema, CONST_DATABASE_SCHEMA_VERSION):
                ...
                try:
                    ...
                    log_info(
                        logger,
                        f"[INFO] Database schema version updated to {CONST_DATABASE_SCHEMA_VERSION}",
                    )
                    return True
                except Exception as e:
                    log_error(
                        logger, f"[ERROR] Failed to write schema version to file: {e}"
                    )
                    return False
            else:
            ...
            log_info(
                logger,
                f"[INFO] Database schema is up to date (version {CONST_DATABASE_SCHEMA_VERSION})",
            )
            return True
        ...
        log_error(
            logger,
            f"[ERROR] Unexpected error while checking/updating schema version: {e}",
        )
        return False
...
def update_database_schema(current_version, target_version):
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Executing schema update from version {current_version} to {target_version}",
        )
        ...
        if current_version_int < 12:
            log_info(
                logger, "[INFO] Version is less than 12, recreating explore view table"
            )
        ...
        if current_version_int < 13:
            log_info(
                logger, "[INFO] Version is less than 13, recreating explore view table"
            )
        ...
        if current_version_int < 14:  # RESUME HERE #TODO: Implement migration logic
            log_info(
                logger,
                "[INFO] Version is less than 14, migrating configuration to dedicated configuration database",
            )
        ...
        if current_version_int < 15:  # RESUME HERE #TODO: Implement migration logic
            log_info(
                logger,
                "[INFO] Version is less than 15, migrating localhosts to dedicated configuration database",
            )
        ...
        if current_version_int < 16:  # RESUME HERE #TODO: Implement migration logic
            log_info(
                logger,
                "[INFO] Version is less than 16, adding column to localhosts table",
            )
            migrate_configurations_schema15_to_schema16()
        # delete_all_records( "dbperformance")

        if current_version_int < 17:  # RESUME HERE #TODO: Implement migration logic
            log_info(
                logger,
                "[INFO] Version is less than 17, adding column to localhosts table",
            )
        ...
        if current_version_int < 21:
            ...
            if not migrate_configurations_schema20_to_schema21():
                return False
        ...
        log_error(
            logger, f"[ERROR] Invalid version format, could not convert to integer: {e}"
        )
        return False
    except Exception as e:
...
def migrate_configurations_schema13_to_schema14():
    ...
    log_info(
        logger,
        "[INFO] Starting configuration migration from CONSOLIDATED_DB to CONFIGURATION_DB",
    )
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Inserted {len(rows)} configuration entries into CONFIGURATION_DB",
        )
        ...
        log_error(
            logger, f"[ERROR] Unexpected error during configuration migration: {e}"
        )
        return False
    finally:
...
def migrate_configurations_schema14_to_schema15():
    ...
    log_info(
        logger,
        "[INFO] Starting localhost migration from CONSOLIDATED_DB to LOCALHOSTS_DB",
    )
    ...
    try:
        ...
        insert_sql = (
            f"INSERT INTO localhosts ({', '.join(columns)}) VALUES ({placeholders})"
        )
        cursor_localhosts.executemany(insert_sql, rows)
        conn_localhosts.commit()

        log_info(
            logger, f"[INFO] Migrated {len(rows)} localhost entries into LOCALHOSTS_DB"
        )
...
def migrate_configurations_schema15_to_schema16():
    ...
    logger = logging.getLogger(__name__)
    log_info(
        logger, "[INFO] Adding 'whitelisted' column to localhosts table (default 0)"
    )
    ...
    try:
        ...
        if "whitelisted" not in columns:
            cursor.execute(
                "ALTER TABLE localhosts ADD COLUMN whitelisted INTEGER DEFAULT 0"
            )
            conn.commit()
            log_info(logger, "[INFO] 'whitelisted' column added to localhosts table")
        else:
            log_info(
                logger, "[INFO] 'whitelisted' column already exists in localhosts table"
            )
...
def migrate_configurations_schema16_to_schema17():
    """
    Adds 'src_sandoname' and 'dst_sandoname' columns to the explore table if they do not exist.
    """
    logger = logging.getLogger(__name__)
    try:
        conn = connect_to_db("explore")
        cursor = conn.cursor()
        # Check for src_sandoname column
        cursor.execute("PRAGMA table_info(explore)")
        columns = [row[1] for row in cursor.fetchall()]
        if "src_sandoname" not in columns:
            cursor.execute("ALTER TABLE explore ADD COLUMN src_sandoname TEXT")
            conn.commit()
            log_info(logger, "[INFO] Added 'src_sandoname' column to explore table.")
        else:
            log_info(
                logger, "[INFO] 'src_sandoname' column already exists in explore table."
            )
        if "dst_sandoname" not in columns:
            cursor.execute("ALTER TABLE explore ADD COLUMN dst_sandoname TEXT")
            conn.commit()
            log_info(logger, "[INFO] Added 'dst_sandoname' column to explore table.")
        else:
            log_info(
                logger, "[INFO] 'dst_sandoname' column already exists in explore table."
            )
        disconnect_from_db(conn)
        return True
    except Exception as e:
...
def init_configurations_from_sitepy():
    ...
    try:
        ...
        log_info(logger, "[INFO] Default configurations initialized successfully.")
        disconnect_from_db(conn)
    except sqlite3.Error as e:
        ...
        if config_dict:
            ...
            try:
                if counts["last_flow_seen"]:
                    last_flow_time = datetime.strptime(
                        counts["last_flow_seen"], "%Y-%m-%d %H:%M:%S"
                    )
                    ...
                    if time_difference <= 300:
                        ...
                        log_info(
                            logger,
                            f"[INFO] System is healthy: Last flow seen {time_difference:.0f} seconds ago",
                        )
                    else:
                        log_warn(
                            logger,
                            f"[WARN] System health check failed: Last flow {time_difference:.0f} seconds ago, "
            ...
            log_info(
                logger,
                f"[INFO] Retrieved flow statistics from configuration: "
                f"Packets: {counts['total_packets']}, Flows: {counts['total_flows']}, "
                f"Bytes: {counts['total_bytes']}, Last seen: {counts['last_flow_seen']}",
            )
        else:
            log_warn(
                logger, "[WARN] Could not retrieve configuration for flow statistics"
            )
    except sqlite3.Error as e:
...
def store_version():
    """
    Store the current version from src.const.py in the configuration database
    ...
    logger = logging.getLogger(__name__)
    try:
        # Import the version from src.const.py
...
def store_machine_unique_identifier():
    ...
    try:
        ...
        log_info(
            logger, f"[INFO] Machine unique identifier stored successfully: {unique_id}"
        )
        return True
        ...
        log_error(
            logger,
            f"[ERROR] Database error while storing machine unique identifier: {e}",
        )
        return False
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while storing machine unique identifier: {e}",
        )
        return False
...
def get_machine_unique_identifier_from_db():
    ...
    try:
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving machine unique identifier: {e}",
        )
        return None
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving machine unique identifier: {e}",
        )
        return None
    finally:
        if "conn" in locals() and conn:
...
def test_database_online(db_path):
    ...
    try:
        ...
        log_error(
            logger, f"[ERROR] Unexpected error during database connection test: {e}"
        )
        return False
...
def get_p95_execution_times():
    ...
    try:
        ...
        log_error(
            logger, f"[ERROR] Database error while retrieving p95 execution times: {e}"
        )
        return {}
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving p95 execution times: {e}",
        )
        return {}
    finally:
        if "conn" in locals() and conn:
