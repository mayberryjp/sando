...
def get_services_by_port(port_number):
    ...
    try:
        ...
        for row in rows:
            protocol = row[0]
            services_dict[protocol] = {"service_name": row[1], "description": row[2]}

        log_info(
            logger,
            f"[INFO] Retrieved {len(services_dict)} service entries for port {port_number}.",
        )
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving services for port {port_number}: {e}",
        )
        return {}
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving services for port {port_number}: {e}",
        )
        return {}
    finally:
...
def get_all_services_database():
    ...
    try:
        ...
        log_info(
            logger, f"[INFO] Retrieved {len(rows)} service entries from the database."
        )
        return rows
...
def insert_service(port_number, protocol, service_name, description):
    ...
    try:
        ...
        log_error(
            logger, f"[ERROR] Unexpected error while inserting service record: {e}"
        )
        return False
    finally:
        if "conn" in locals() and conn:
...
def insert_services_bulk(service_records):
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Successfully bulk inserted/updated {record_count} service records in {execution_time:.2f} ms",
        )
...
def create_services_db():
    ...
    try:
        ...
        log_info(
            logger, "[INFO] Downloading IANA service names and port numbers CSV..."
        )
        ...
        if response.status_code != 200:
            log_error(
                logger,
                f"[ERROR] Failed to download IANA services CSV: {response.status_code}",
            )
            return False
            ...
            try:
                ...
                if batch_count >= batch_size:
                    ...
                    if success:
                        ...
                        log_info(
                            logger,
                            f"[INFO] Processed batch of {count} service entries (total: {total_count})...",
                        )
                    else:
                        log_error(
                            logger,
                            f"[ERROR] Failed to insert batch of {len(service_records)} records",
                        )
        ...
        if service_records:
            ...
            if success:
                ...
                log_error(
                    logger,
                    f"[ERROR] Failed to insert final batch of {len(service_records)} records",
                )

        log_info(
            logger,
            f"[INFO] Successfully loaded {total_count} IANA service entries into the database.",
        )
        return True
        ...
        log_error(
            logger, f"[ERROR] Database error while creating services database: {e}"
        )
        return False
    except Exception as e:
