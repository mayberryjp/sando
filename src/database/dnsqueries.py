...
def insert_dns_query(client_ip, domain, times_seen, datasource):
    ...
    try:
        ...
        log_error(
            logger, f"[ERROR] Database error while inserting DNS query record: {e}"
        )
        return False
    except Exception as e:
        log_error(
            logger, f"[ERROR] Unexpected error while inserting DNS query record: {e}"
        )
        return False
    finally:
...
def get_client_dns_queries(client_ip):
    ...
    try:
        ...
        conn = connect_to_db("dnsqueries")
        if not conn:
            log_error(logger, "[ERROR] Unable to connect to Pi-hole database.")
            return []
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving DNS queries for client IP {client_ip}: {e}",
        )
        return []
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving DNS queries for client IP {client_ip}: {e}",
        )
        return []
    finally:
        if "conn" in locals() and conn:
...
def insert_dns_queries_batch(queries, datasource):
    ...
    try:
        ...
        if not conn:
            log_error(
                logger,
                "[ERROR] Unable to connect to dnsqueries database for batch insert.",
            )
            return False, 0
        ...
        log_info(
            logger,
            f"[INFO] Successfully batch inserted/updated {record_count} DNS query records",
        )
        ...
        log_error(
            logger, f"[ERROR] Unexpected error during dnsqueries batch insert: {e}"
        )
        return False, 0
    finally:
        if "conn" in locals() and conn:
...
def update_dns_query_response(response, id):
    ...
    try:
        ...
        log_error(
            logger, f"[ERROR] Database error while updating DNS query response: {e}"
        )
        return False
    except Exception as e:
        log_error(
            logger, f"[ERROR] Unexpected error while updating DNS query response: {e}"
        )
        return False
    finally:
        if "conn" in locals() and conn:
...
def get_dnsqueries_without_responses():
    ...
    try:
        ...
        log_info(
            logger, f"[INFO] Retrieved {len(rows)} DNS queries with empty responses"
        )
        return rows
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving DNS queries without responses: {e}",
        )
        return []
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving DNS queries without responses: {e}",
        )
        return []
    finally:
        if "conn" in locals() and conn:
...
def get_ip_to_domain_mapping():
    ...
    try:
        ...
        if not conn:
            log_error(
                logger, "[ERROR] Unable to connect to database for IP-domain mapping."
            )
            return {}
        ...
        """
        rows, _ = run_timed_query(
            cursor, query, description="get_ip_to_domain_mapping", fetch_all=True
        )
        ...
        log_info(
            logger,
            f"[INFO] Created IP-to-domain mapping with {len(ip_to_domain)} entries",
        )
        return ip_to_domain

    except sqlite3.Error as e:
        log_error(
            logger, f"[ERROR] Database error while creating IP-to-domain mapping: {e}"
        )
        return {}
    except Exception as e:
        log_error(
            logger, f"[ERROR] Unexpected error while creating IP-to-domain mapping: {e}"
        )
        return {}
    finally:
        if "conn" in locals() and conn:
