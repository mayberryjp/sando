...
def insert_custom_tag(
    tag_id, src_ip, dst_ip, dst_port, protocol, tag_name="", enabled=1
):
    """
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Successfully inserted new custom tag with ID {tag_id} in {insert_time:.2f} ms",
        )
        return True, tag_id
...
def get_custom_tags():
    ...
    try:
        ...
        log_info(
            logger, f"[INFO] Retrieved {len(customtags)} active custom tag entries"
        )
...
def import_custom_tags(config_dict):
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Imported {len(tag_entries)} custom tag entries into the database.",
        )
