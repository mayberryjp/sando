...
def delete_old_traffic_stats():
    ...
    try:
        ...
        cutoff_date = (datetime.now() - timedelta(days=purge_time_delta)).strftime(
            "%Y-%m-%d"
        )
        ...
        log_info(
            logger,
            f"[INFO] Deleted records older than {cutoff_date} from trafficstats table.",
        )
        return True

    except sqlite3.Error as e:
        log_error(
            logger, f"[ERROR] Database error while deleting old traffic stats: {e}"
        )
        return False
    except Exception as e:
        log_error(
            logger, f"[ERROR] Unexpected error while deleting old traffic stats: {e}"
        )
        return False
    finally:
...
def get_all_ips_traffic_status():
    ...
    try:
        ...
        log_info(
            logger,
            f"[INFO] Successfully determined traffic status for {len(ip_traffic_status)} IPs ({len(active_ips)} active)",
        )
        return ip_traffic_status
...
def get_traffic_stats_for_ip(ip_address):
    ...
    try:
        ...
        traffic_rows, traffic_query_time = run_timed_query(
            ...
            traffic_query,
            (ip_address,),
            description="get_traffic_stats_for_ip_get_traffic_stats",
        )
        ...
        alert_rows, alerts_query_time = run_timed_query(
            ...
            alerts_query,
            (ip_address,),
            description="get_traffic_stats_for_ip_get_alert_counts",
        )
        ...
        for row in traffic_rows:
            timestamp = row[1]
            traffic_data[timestamp] = {"total_packets": row[2], "total_bytes": row[3]}
            ...
            traffic_stats.append(
                {
                    ...
                    "timestamp": timestamp,
                    "total_packets": (
                        traffic_for_hour["total_packets"] if traffic_for_hour else None
                    ),
                    "total_bytes": (
                        traffic_for_hour["total_bytes"] if traffic_for_hour else None
                    ),
                    "alerts": alert_count,
                }
            )
        ...
        log_info(
            logger,
            f"[INFO] Generated {len(traffic_stats)} traffic stats entries (including null entries) "
        ...
        log_error(
            logger,
            f"[ERROR] Database error while retrieving traffic stats for IP {ip_address}: {e}",
        )
        return []
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error while retrieving traffic stats for IP {ip_address}: {e}",
        )
        return []
    finally:
