...
from src.integrations.dns import (  # Import the dns_lookup function from dns.py
    dns_lookup,
    resolve_empty_dns_responses,
)
from src.integrations.geolocation import load_geolocation_data
from src.integrations.ipasn import create_asn_database
from src.integrations.nmap_fingerprint import os_fingerprint
from src.integrations.piholedhcp import (
    get_pihole_dhcp_leases,
    get_pihole_network_devices,
)
from src.integrations.piholedns import get_pihole_ftl_logs
from src.integrations.reputation import import_reputation_list, load_reputation_data
from src.integrations.services import create_services_db, get_all_services
...
def classify_client(machine_identifier, client_data):
    ...
    try:
        log_info(
            logger,
            f"[INFO] Sending client data to classification API for machine {machine_identifier}",
        )
        ...
        log_info(
            logger,
            f"[INFO] Successfully received classification for machine {machine_identifier}",
        )
        ...
        log_error(
            logger,
            f"[ERROR] HTTP error when classifying machine {machine_identifier}: {e}",
        )
        log_error(
            logger,
            f"[ERROR] Response content: {e.response.text if hasattr(e, 'response') else 'No response'}",
        )
    except requests.exceptions.ConnectionError as e:
        log_error(
            logger,
            f"[ERROR] Connection error when classifying machine {machine_identifier}: {e}",
        )
    except requests.exceptions.Timeout as e:
        log_error(
            logger,
            f"[ERROR] Timeout when classifying machine {machine_identifier}: {e}",
        )
    except requests.exceptions.RequestException as e:
        log_error(
            logger,
            f"[ERROR] Request error when classifying machine {machine_identifier}: {e}",
        )
    except json.JSONDecodeError as e:
        log_error(
            logger,
            f"[ERROR] Invalid JSON in classification response for machine {machine_identifier}: {e}",
        )
    except Exception as e:
        log_error(
            logger,
            f"[ERROR] Unexpected error when classifying machine {machine_identifier}: {e}",
        )
...
def copy_flows_to_newflows():
    ...
    for source_db in CONST_TEST_SOURCE_DB:
        try:
            ...
            log_info(logger, "[INFO] Preparing to insert flows into newflows table")
...
def main():
    ...
    if not os.path.exists(CONST_CONFIGURATION_DB):
        ...
        log_info(
            logger, f"[INFO] Configuration database found at {CONST_CONFIGURATION_DB}."
        )
    ...
    log_info(
        logger,
        f"NTP config settings are : {config_dict.get('BypassLocalNtpDetection', '0')}, {config_dict.get('ApprovedLocalNtpServersList', '')}, {config_dict.get('ApprovedNtpStratumServersList', '')}",
    )
    ...
    log_info(
        logger,
        f"DNS config settings are : {config_dict.get('BypassLocalDnsDetection', '0')}, {config_dict.get('ApprovedLocalDnsServersList', '')}, {config_dict.get('ApprovedAuthoritativeDnsServersList', '')}",
    )
    ...
    tagged_rows = [
        [row[col] if col in row else None for col in column_names]
        for row in tagged_rows_as_dicts
    ]
    ...
    detection_durations["update_local_hosts"] = int(
    ...
    detection_durations["detect_new_outbound_connections"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    router_flows_detection(filtered_rows, config_dict)
    detection_durations["router_flows_detection"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    foreign_flows_detection(filtered_rows, config_dict)
    detection_durations["foreign_flows_detection"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    local_flows_detection(filtered_rows, config_dict)
    detection_durations["local_flows_detection"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_dead_connections(config_dict)
    detection_durations["detect_dead_connections"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_unauthorized_dns(filtered_rows, config_dict)
    detection_durations["detect_unauthorized_dns"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_unauthorized_ntp(filtered_rows, config_dict)
    detection_durations["detect_unauthorized_ntp"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_incorrect_ntp_stratum(filtered_rows, config_dict)
    detection_durations["detect_incorrect_ntp_stratum"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_incorrect_authoritative_dns(filtered_rows, config_dict)
    detection_durations["detect_incorrect_authoritative_dns"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_vpn_traffic(filtered_rows, config_dict)
    detection_durations["detect_vpn_traffic"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_many_destinations(filtered_rows, config_dict)
    detection_durations["detect_many_destinations"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_high_risk_ports(filtered_rows, config_dict)
    detection_durations["detect_high_risk_ports"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_port_scanning(filtered_rows, config_dict)
    detection_durations["detect_port_scanning"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    detection_durations["detect_tor_traffic"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_high_bandwidth_flows(filtered_rows, config_dict)
    detection_durations["detect_high_bandwidth_flows"] = int(
        (datetime.now() - start).total_seconds()
    )

    start = datetime.now()
    detect_custom_tag(filtered_rows, config_dict)
    detection_durations["detect_custom_tag"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    detection_durations["detect_geolocation_flows"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    detection_durations["retrieve_pihole_dns_query_logs"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    detection_durations["resolve_empty_dns_responses"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    detection_durations["detect_reputationlist_flows"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    detection_durations["fetch_services_flow"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    detection_durations["create_dns_keyvalue"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    detection_durations["populate_master_flow_view"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    start = datetime.now()
    dns_results = dns_lookup(
        localhosts, config_dict["ApprovedLocalDnsServersList"].split(","), config_dict
    )
    ...
    for result in dl_results:
        ...
        combined_results[ip].update(
            {
                "lease_hostname": result.get(
                    "lease_hostname", combined_results[ip].get("lease_hostname")
                ),
                "lease_hwaddr": result.get(
                    "lease_hwaddr", combined_results[ip].get("lease_hwaddress")
                ),
                "lease_clientid": result.get(
                    "lease_clientid", combined_results[ip].get("lease_clientid")
                ),
            }
        )
    detection_durations["discovery_pihole_dhcp_leases"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    for result in nd_results:
        ...
        combined_results[ip].update(
            {
                "dhcp_hostname": result.get(
                    "dhcp_hostname", combined_results[ip].get("dhcp_hostname")
                ),
                "mac_address": result.get(
                    "mac_address", combined_results[ip].get("mac_address")
                ),
                "mac_vendor": result.get(
                    "mac_vendor", combined_results[ip].get("mac_vendor")
                ),
            }
        )
    detection_durations["discovery_pihole_network_devices"] = int(
        (datetime.now() - start).total_seconds()
    )
    ...
    if config_dict.get("DiscoveryNmapOsFingerprint", 0) == 1:
        ...
        sub_localhosts = list(localhosts)[
            :1
        ]  # Slice the list to include only the first 3 hosts
        ...
        for result in nmap_results:
            ...
            combined_results[ip].update(
                {
                    "os_fingerprint": result.get(
                        "os_fingerprint", combined_results[ip].get("os_fingerprint")
                    ),
                }
            )
        detection_durations["discovery_nmap_os_fingerprint"] = int(
            (datetime.now() - start).total_seconds()
        )
